  ...
  return (
    <div className="container mx-auto py-8 max-w-6xl px-4">
      <Card className="mb-10 rounded-3xl shadow-md overflow-hidden">
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row justify-between items-start gap-6">
            <div className="flex gap-6">
              ...
              <div className="flex flex-col justify-center">
                <h1 className="font-semibold text-[28px] mb-2">Asutosh Sidhya</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6 mt-8">
            ...
            <InfoItem icon={<CalendarIcon />} label={<>2023 <span className="mx-2">to</span> 2027</>} />
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {hobbies.map((hobby, index) => (
                <span key={index} className="text-lg font-light">{hobby}</span>
              ))}
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex flex-wrap gap-6 mb-10">
        <OverviewCard color="rgba(8, 43, 30, 0.3)" img="/image-13.png" count={10} label="Courses Completed" />
        <OverviewCard color="rgba(202, 17, 140, 0.3)" img="/image-14.png" count={10} label="Ongoing Courses" />
      </div>
      ...
      <div className="flex items-center mb-4">
        ...
        <Badge className="ml-4 bg-red-500/40 text-black text-lg px-3 py-1 rounded-full">5</Badge>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {badgeData.map((badge, index) => (
          <BadgeCard key={index} color={badge.color} img={badge.img} title={badge.title} subtitle={badge.subtitle} />
        ))}
      </div>
    </div>
  );
